...
mle_opt <- optimize(mle, interval = c(0,10))
mle_opt
...
# 최적화 수행하여 모수 추정
lse_opt <- optimize(lse,interval = c(0,10))
lse_opt
...
hist(x, col = 'blue', breaks = 50, main = "Density of Simulated Data", probability = T)
curve(dexp(x, rate = mle_opt$minimum), col='red', add = T)
curve(dexp(x, rate = lse_opt$minimum), col='green', add = T)
